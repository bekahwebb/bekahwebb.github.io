...
---
# Introduction:
 I am going to do a deeper exploratory data analysis on my camp data than I did in my last camp blog post.  I mentioned in my last blog post that I want to examine potential correlations between some of the numeric data.  I am curious about what I will find in examining this data further.

# Examining the suprise data results for top Camp Costs.
In my last blog post I shared a histogram with the results for the top camp costs.  I could see from the head of my data results that there were some camp costs close to $100 which suprised me there were campgrounds that expensive.  When I did an overall data analysis of top costs, I was shocked.
![Alt text](image-2.png)
When I looked at the description of Camp Meadow, then I saw that it sleeps 140 people so it makes more since that it would be $1000 for the night of camping, and in searching for Camp Meadow on the internet, then I also saw that it's a rugged cabin they sleep in with amenities like a pool as well.  The top 7 in my bargraph are more 'glamping' than camping.


# Checking to see if my data is scale free.
I converted my camp_data to an array so I could use logarithmic bins to show that the cost is scale free. Scale free data is observed in real life data is heavy tailed vs. normal distributions that have small tails.  It shows that there is a particular shape, a Mathew effect, he who has more should be given, and he who has little it should be taken away, the talent parable.  Power law distribution, creators of the moon, solar flairs, freq. Of words, freq. Of family names etc.  Here is the code I used to get my logarithmic bins histogram:
``` 
import numpy as np
...
# Step 2: Checking for correllation and conversely inverse correlation

```
# Here's code to check for correlation
 camp_data.corr()
...
Our scatterplot results for cost by region are not linear so we know there is not a strong relationship, or correlation between change in region and change in cost.  We can also see that the data by region is centered between $0 and close to $100. There are points for the outliers by region in this plot.
...
The Midwest area in the US has the top average cost by region 55, eg Minnesota.  The Northeast has the lowest cost by region 20, eg. Washington DC which makes since.  Minnesota is one of the great lake states where you can imagine there are some great camping areas, and DC on the flip side doesn't have many campgrounds or National Parks nearby.

# Looking at boxplots by region of Utah and surrounding neighbors

I mentioned in my first blog post that Utah is typically where my family goes camping.  I thought it would be fun to compare Utah with surrounding neighbor states to compare campground cost by region with BYU's state and bordering states.
...
Left to right the box plots are representing the Zip code regions of AZ, CO, NV, UT, WY, ID, and NM.  There are a few outliers in UT, WY, and ID.  ID has the highest cost for camping.  The order of highest campground cost to lowest is ID, UT, WY, CO, NM, AZ, and NV has missing values for the zip code so we don't have the campground cost represented. UT, WY, and NM are positively skewed, and AZ, and ID are negatively skewed.  On a personal note, I have been to the following National parks: AZ, the Grand Canyon, UT, Arches, Zion, Bryce Canyon and the Canyonlands, and ID, Yellowstone.  I have enjoyed camping at most of these locations, the exception was the Grand Canyon, where my family stayed in a motel, and Zion, my husband and I slept in the car which may be considered it's own free camping.

# Examining the Park Codes Frequency

I wanted to see how frequently the Park Codes were used throughout my data.  I thought it would be fun to look at a word cloud to see the most frequently used Park Codes.
...
# Conclusion:
 ...
 It has been interesting to examine the National Park Service campground data.  I have really enjoyed the opportunity to go through this process of having a question that I wanted data to answer.  It's felt like I have been a detective decoding the National Park data by different column variables eg the city column or by zip codes.  I liked being suprised by the cost outliers of the campgrounds that were over $100 a night.  The Streamlit app was fun to design and interact with.  It was part of my detective decoding, searching the data by different column names to find out more information about the different campgrounds in the National Parks across the nation.

 We live in a beautiful world, now go camp it.
